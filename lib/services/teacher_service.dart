...
class TeacherService {
  ...
  Future<bool> requestManualDoorUnlock(String teacherUid, String teacherName, String teacherID) async {
    try {
      // Ensure any previous command/request is cleared before issuing a new one
      await Future.wait([
        _database.ref('door_unlock_commands/$teacherID').remove(),
        _database.ref('door_unlock_requests/$teacherID').remove(),
      ]);

      // Create unlock request consumed by Cloud Function
      final unlockRequest = {
        'status': 'pending',
  ...
  Stream<String?> getManualUnlockStatus(String teacherUid) {
    return _database
      ...
      if (teacherIDEvent.snapshot.exists) {
        ...
        return _database
            .ref('door_unlock_commands/$teacherID/status')
            .onValue
            .map((statusEvent) {
  ...
  Stream<Map<String, dynamic>?> getManualUnlockData(String teacherID) {
    return _database
        .ref('door_unlock_commands/$teacherID')
        .onValue
        .map((event) {
